...
d3.csv('./data/gdp.csv', function(error, data) {
  ...
  group
    ...
    .attr('r', function(d) {
      return 20 * Math.pow(d.co21980 / 1000000 / 3.14, 0.5);
    })
    .attr('cx', function(d) {
      return x(d.loggdppc1980);
    })
    .attr('cy', function(d) {
      return y(d.logco2pc1980);
    });
  ...
  legend
    .append('circle')
    .attr('cy', function(d) {
      return -20 * Math.pow(d / 1000000 / 3.14, 0.5);
    })
    .attr('r', function(d) {
      return 20 * Math.pow(d / 1000000 / 3.14, 0.5);
    });
  ...
  legend
    .append('text')
    .attr('y', function(d) {
      return -40 * Math.pow(d / 1000000 / 3.14, 0.5);
    })
    .attr('dy', '1.3em')
    .text(d3.format('.1s'));
      ...
      tooltip
        .html(
          '<p><strong>' +
            ...
            year +
            '</strong></p>' +
            '<p>CO<sub>2</sub> emissions: <strong>' +
            numberFormatDetailed(d[selectedCO2] / 1000000) +
            ' million kilotons</strong></p>' +
            '<p>GDP per capita: <strong>$' +
            numberFormat(d[selectedGDP2]) +
            '</strong></p>' +
            '<p>CO<sub>2</sub> per capita: <strong>' +
            numberFormatDetailed(d[selectedCO2pc2]) +
...
function updateScatter() {
  ...
  d3.selectAll('.scatter')
    ...
    .ease('linear')
    .attr('r', function(d) {
      return 20 * Math.pow(d[selectedCO2] / 1000000 / 3.14, 0.5);
    })
    .attr('cx', function(d) {
      return x(d[selectedGDP]);
    })
    .attr('cy', function(d) {
      return y(d[selectedCO2pc]);
    });
